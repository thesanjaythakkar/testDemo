...
class ProfileVC: UIViewController {
    ...
    var user: Users!
    var dataProvider = DataProvider(persistentContainer: CoreDataStack.shared.persistentContainer, repository: APIDataManager.shared)
    ...
    @IBAction func tappedOnSave(_ sender: UIButton?)
    {
        ...
        if let value = txtNote.text, value.trimmingCharacters(in: .whitespacesAndNewlines).count > 0
        {
            ...
            dataProvider.saveUser(user: user, note: note) { (isDone) in
